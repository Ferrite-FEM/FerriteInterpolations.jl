...
@testset "Bernstein" begin
    ...
    # DOF) account for the differing vertex/edge numbering and edge
    # orientation; they were verified against the independently derived
    # Bernstein-coefficient tables in src/bernstein.jl (symfem's Bernstein
    # entity DOFs follow its reference.edges order, with edge DOFs ordered
    # along each symfem edge).
    for (ip, perm) in (
            (Bernstein{RefLine, 1}(), [0, 1]),
            (Bernstein{RefLine, 2}(), [0, 1, 2]),
            (Bernstein{RefLine, 3}(), [0, 1, 2, 3]),
            (Bernstein{RefTriangle, 1}(), [1, 2, 0]),
            (Bernstein{RefTriangle, 2}(), [1, 2, 0, 5, 4, 3]),
            (Bernstein{RefTriangle, 3}(), [1, 2, 0, 7, 8, 6, 5, 3, 4, 9]),
            (Bernstein{RefTetrahedron, 1}(), [0, 1, 2, 3]),
            (Bernstein{RefTetrahedron, 2}(), [0, 1, 2, 3, 4, 7, 5, 6, 8, 9]),
        )
        test_symfem_reference(ip, "Bernstein", Ferrite.getorder(ip), perm)
    end
