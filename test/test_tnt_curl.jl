...
@testset "TNTCurl" begin
    ip = TNTCurl{RefQuadrilateral, 1}()

    # (i) Interpolation-level tests
    test_interpolation_properties(ip)
    test_type_genericity(ip)

    # Live symfem cross-check ("TNTcurl"): signed permutation derived with
    ...
    test_symfem_reference_vector(
        ip, "TNTcurl", 1,
        [(2, 0), (2, 1), (2, 4), (2, 5), (-2, 7), (-2, 6), (-2, 3), (-2, 2), (2, 8), (2, 9), (2, 10)],
    )
