# Bell element (reduced quintic C1 triangle) (https://defelement.org/elements/bell.html,
# DefElement: elements/bell.def).
#
# Bell element (reduced quintic C1 triangle).
#
# STATUS: BLOCKED -- what must change upstream in Ferrite
#
# The element's DOFs include derivative-type functionals (vertex values and vertex first/second derivatives), which are
...
notes in `src/bell.jl`.
"""
struct Bell
    function Bell()
        return error("Bell is not implemented; see the STATUS: BLOCKED notes in src/bell.jl.")
    end
end
