# Morley element (quadratic nonconforming plate triangle) (https://defelement.org/elements/morley.html,
# DefElement: elements/morley.def).
#
# Morley element (quadratic nonconforming plate triangle).
#
# STATUS: BLOCKED -- what must change upstream in Ferrite
#
# The element's DOFs include derivative-type functionals (edge-midpoint normal derivatives), which are
...
notes in `src/morley.jl`.
"""
struct Morley
    function Morley()
        return error("Morley is not implemented; see the STATUS: BLOCKED notes in src/morley.jl.")
    end
end
